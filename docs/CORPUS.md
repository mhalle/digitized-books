...
| ★ | Cunningham's Manual of Practical Anatomy (Robinson rev., 6th ed.) | `kw6vt8gv` | 1914 | inc → PD override: pre-1929 US PD | Dissection-room companion. The work was originally published as 2 volumes (Vol. 1: limbs; Vol. 2: thorax/abdomen/head/neck/brain). The Wellcome scan is **752 canvases with continuous pagination** (body p.1–681 then index p.652+) — there is no internal Title-Page/Cover marking a volume boundary, so the IIIF view is best treated as one continuous text. Earlier CORPUS draft asserting "two 'Cover' ranges mark the volume break" was wrong — the two Cover ranges are the outer front/back covers of the bound volume. 34 duplicate page-number pairs in the page_numbers table are plate fold-out scans, not volume restarts. The 1920 BHL edition (BHL title 30040) is a close alternative if you want the post-WWI revision. |
...
| ★ | Quain's Elements of Anatomy, 11th ed | `gw75hbbr` | 1908–09 | cc-by-nc (work itself PD) | Latest pre-1929 edition. **Wellcome holds only 3 of the 11th ed's 7 published parts** — Vol I Embryology (`_v1`), Vol III Pt I Neurology/CNS (`_v2`; redundant scan `_v3` cleared), Vol III Pt II Peripheral Nerves & Sense Organs (`_v4`). Missing from this Wellcome record: Vol II Pt I Microscopic anatomy (1912), Vol II Pt II Splanchnology (1914), Vol IV Pt I Osteology/Arthrology (1915), Vol IV (later) Myology/Angeiology (1923). Use the 10th ed below as a complement for those topics. |
| ★ | Quain's Elements of Anatomy, 10th ed | `jcsn4x8q` | 1890–96 | pdm | **Complete set: 3 volumes in 9 parts**, same Schäfer/Thane editorial team as the 11th. Ingested + outlined 2026-05-15 (`_v1..._v9`, 2,288 canvases, 1,250 outline entries). Subjects: Vol I Pt 1 Embryology, Vol I Pt 2 Histology, Vol II Pt 1 Osteology, Vol II Pt 2 Arthrology/Myology/Angeiology, Vol III Pt 1 Spinal Cord & Brain, Vol III Pt 2 Peripheral Nerves, Vol III Pt 3 Sense Organs, Vol III Pt 4 Splanchnology, Appendix. **Replaces 11th-ed gaps for histology/splanchnology/osteology/arthrology/myology/angeiology coverage.** Earlier CORPUS draft mislabelled this as "8th ed" — Wellcome catalogue API explicitly returns `edition: "10th ed. /"`. Caveat: `_v5` scan is missing TOC pages i–iii (only the page-iv continuation survives); body-header fallback applied to recover Spinal Cord / Medulla / Cerebellum / Mesencephalon / Cerebral Hemispheres sections (flagged in `notes`). v4 has one TOC printed-typo (Abdominal Aorta p.459 → 457) and v8 has two (Structure p.29 → 297; Cement/Alveoli swap) patched in the verbatim. |
| | Quain's Elements of Anatomy, 8th ed | `napwfjw9` | 1876 | pdm | 2 volumes, pre-Schäfer. Skip — the 10th and 11th together cover this material with more recent scholarship. Earlier CORPUS draft mislabelled this as "7th ed". |
...
6. **Wellcome record completeness gotchas.** A Wellcome work record marked "Online" doesn't always mean the IIIF manifest contains a full scan. Three patterns to watch for:
   ...
   - **Redundant scans within a Collection.** Some Wellcome Collections contain two distinct scans of the same physical volume — different microfilm sources, different scan dates, slightly different canvas counts, but identical body content. Discovered during outlining: Quain's `gw75hbbr_v2` (444 canvases) and `gw75hbbr_v3` (468 canvases) both encode Vol III: The Nervous System — same TOC, same body, different scans. `_v3` outline was cleared after this was confirmed; outline `_v2` is the kept copy. When outlining a Wellcome Collection child, sanity-check against sibling children before producing a payload — if the TOC is identical, the work is a redundant scan and need not be outlined separately.
...
**`corpus/wellcome/` — 73 sqlite indexes on disk, 411 MB; 59 in the active corpus, 14 redundancy mirrors:**

Active (59 dbs, ~32,496 canvases):
- 35 single-manifest works (the bulk of Tier 1–7 ★ picks with full ALTO/FTS) — ~19,520 canvases, ~153,530 text blocks, ~10,993 illustration regions, ~50.7M chars
- 22 child indexes from Wellcome multi-vol Collections, named `<work_id>_v<N>.sqlite`:
  ...
  Sobotta Histologie (`v43geect_v1..v2`) — ~12,547 canvases, ~108,987 blocks, ~6,147 illus, ~30.9M chars
- 2 metadata-only indexes for pre-1600 works with no ALTO at all (Vesalius *Epitome* `g6b6smge`, Valverde `nrtzmcfn`) — ~429 canvases

Redundancy mirror — **excluded from the active corpus, kept on disk only as a fallback if Heidelberg goes offline:**
- 14 Bourgery image-only indexes (`p747b7vs_v1..v14`, built with `--no-ocr` because the Wellcome ALTO endpoint is broken) — ~5,140 canvases, 0 text. Heidelberg's Bourgery (16 vols below) is canonical: it has clean OCR on the text vols, higher-fidelity colored chromolithographs on the atlas vols, and Wellcome adds nothing retrievable on top. Don't build indexes, embeddings, or cross-references off these dbs unless Heidelberg is down.

Active totals (wellcome): ~32,496 canvases, ~262,517 text blocks, ~17,140 illustration regions, ~81.5M chars of OCR.

**`corpus/heidelberg/` — 22 sqlite indexes, 69 MB:**

- 8 Bourgery text vols (`bourgey1832bd1_1` + `bourgery1834bd2_1`...`bourgery1854bd8_1`) — ~2,312 canvases, ~15.1M chars of French OCR
- 8 Bourgery atlas vols (`bourgey1831bd1_2`...`bourgey1844bd8_2`) — ~1,485 canvases with the colored chromolithographs and Planche-N labeled plates, ~2.5M chars of caption/label OCR
- 6 Henle *Hand-Atlas* vols (`henle1871bd1`...`henle1877bd6`) — ~536 canvases, ~324K chars of caption OCR
...
row a single navigable unit with explicit canvas range and printed
page label. The schema, CLI, and JSON Schema are documented in
`docs/OUTLINE.md`. The `build-outline` skill (under
`skills/build-outline/`) drives the production of these outlines from
...
1. **Printed-TOC parse** (default, ~30 works). The book has a usable
   front-matter or back-matter contents page; transcribe verbatim and
   resolve printed pages to canvases.
2. **Body-header fallback** (~5 works). When the printed TOC is
   incomplete (Quain v1) or absent (Spalteholz), mine the body's
   running headers (`UROGENITAL SYSTEM 177`) to recover section
   boundaries. Synthesized entries are flagged with `notes`.
3. **Figure-caption extraction** (~6 atlas works, enrichment pass).
   For plate atlases, mine `text_blocks` for `Fig.N. Title` /
   `Planche N` / `Tafel <Roman>` patterns and splice each named figure
   as a child of its section. Three caption-block shapes documented:
...
| Cajal *Histologie* v2 | `akfqm42j_v2` | fr | 596 | 2 | TOC, back-matter |
| Morris 1914 | `ad56hqjs` | en | 530 | 5 | TOC, 13 systems |
| Sappey v2 | `h9n43hma_v2` | fr | 497 | 5 | TOC, back-matter |
| Bailey 1913 | `n8dz65uv` | en | 489 | 6 | TOC |
...
1. **Wellcome page_numbers collision bug** — when a Wellcome scan has
...
2. **Heidelberg `ranges.canvas_start/end` are NULL** for several dbs —
   ...
   location falls back to OCR scan; no user-visible impact on outlines.
3. **Wellcome image service 404 on Piersol v1 manifest** (`b32846046_0001`)
